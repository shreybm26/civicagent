  ...
  return <section className="review">
    ...
    {fields.map((field) => { const value = drafts[field.id] ?? ""; const unchanged = value === text(field); const options = FIELD_OPTIONS[field.id]; return <div className="review-row" key={field.id}>
      <label htmlFor={`review-${field.id}`}><span>{field.id.replaceAll("_", " ")}{field.required ? " *" : ""}</span>
        {field.id === "photo" ? <output id={`review-${field.id}`} className="attachment-status">{photoAdded ? "Image attached" : "No image attached"}</output> : options ? <span className="choice-bubbles" id={`review-${field.id}`}>{options.map((option) => <button type="button" className={value === option ? "choice-bubble selected" : "choice-bubble"} key={option} disabled={busy || saving !== null} onClick={() => setDrafts((current) => ({ ...current, [field.id]: option }))}>{option.replaceAll("_", " ")}</button>)}</span> : field.id === "additional_details" ? <textarea id={`review-${field.id}`} value={value} disabled={busy || saving !== null} onChange={(event) => setDrafts((current) => ({ ...current, [field.id]: event.target.value }))} /> : <input id={`review-${field.id}`} value={value} disabled={busy || saving !== null} onChange={(event) => setDrafts((current) => ({ ...current, [field.id]: event.target.value }))} />}
