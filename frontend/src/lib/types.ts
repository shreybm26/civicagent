...
export type SessionView = {session_id:string; state:State; service_id:string|null; schema_version:string; messages?:Message[]; fields:Field[]; evidence?:Evidence[]; agent_message:string|null; service?:{service_id:string;name:string;department:string}|null; receipt?:Receipt|null; error:{code:string;message:string;retryable:boolean}|null};
