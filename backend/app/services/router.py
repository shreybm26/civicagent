...
class ServiceRouter:
    ...
    def route(self, text: str, provider=None) -> RouterResult:
        ...
        if not ranked or ranked[0][1] == 0:
            return RouterResult(message="I can take road, garbage, streetlight, water, or sanitation complaints. Which one is this?")
        if len(ranked) > 1 and ranked[0][1] == ranked[1][1]:
            return RouterResult(message="I'm not sure which service fits. Describe it briefly?")
        return RouterResult(service_id=ranked[0][0], confidence=min(0.95, .7 + .1 * ranked[0][1]), needs_clarification=False, message="Got it — routed to the right service.")
