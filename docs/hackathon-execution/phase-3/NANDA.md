...
- [ ] **N3 (P0, 45 min)** Implement `LocationConfirmation.tsx` under `features/location/` with resolved address, confidence/source, confirm/clarify behavior, and no fabricated map claim; a static mini-map placeholder is acceptable.
...
- [ ] **N6 (P0, 60 min)** Add integration tests for session -> messages -> location -> media -> review -> edit -> confirm -> receipt and submission failure -> retry.
- [ ] **N7 (P0, 60 min)** Create the primary E2E spec for Scenario 1 and schema-switch Scenario 2 using deterministic mode; capture exact demo steps and expected visible state.
...
Dependencies: Phase 2 API smoke path and Shrey's component contracts; fixture mode permits parallel progress. API contract: browser calls only the endpoints in [CONTRACTS.md](../CONTRACTS.md), and backend remains authoritative for state/validation. Agent/AI work is N1 provider/error integration and N8 prompt-injection/timeout verification. Backend work is N1-N3/N5; frontend work is N3-N5; integration/testing work is N6-N10. Do not edit `App.tsx`, global CSS, or Shrey-owned feature internals.

## Acceptance criteria

The API and browser agree on state after every action, corrections survive validation, upload/location failures are recoverable, and the primary path reaches a backend receipt. Nanda's frontend code remains isolated under owned feature folders.
