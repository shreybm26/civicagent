...
- [ ] **S1 (P0, 60 min)** Compose `App.tsx` from feature components with one `SessionView` source, API client injection, and clear IDLE/COLLECTING/REVIEWING/COMPLETED/SUBMISSION_FAILED presentation.
...
- [ ] **S8 (P1, 45 min)** Add responsive/accessibility polish: visible focus, labels, status announcements, contrast, mobile stacking, stable upload/input sizes, no overlapping text.
- [ ] **S9 (P0, 60 min)** Add component/interaction tests for all critical states and deterministic API fixtures; test errors, media rejection, correction, submit, receipt, reset, and schema switch.
...
Dependencies: Phase 2 components and `SessionView` fixtures; the UI can be completed before live route wiring. API contract: `frontend/src/lib/api.ts` is the sole caller and follows [CONTRACTS.md](../CONTRACTS.md); UI never creates a receipt or advances state. Agent/AI work is S7 schema-switch rendering and display of router/vision provenance, not provider logic. Backend contribution is S9 malformed-response coverage; frontend work is S1-S8; integration/testing work is S9-S10. Do not edit backend route/config files.

## Acceptance criteria

Judges can understand the current service, fields, provenance, and next action at every turn. The review and receipt are unambiguous, the page works at demo laptop and mobile widths, and no screen depends on hidden manual state setup.
